...
Данная программа представляет собой TCP-сервер, который запускается в качестве службы и предназначен для выполнения вычислительных задач на больших объемах данных. Она использует библиотеку Dask для эффективной работы с параллельными вычислениями.
...
|------------------------|----------------------------------------------|--------------------|------------|------------------|
| init(self, host: str, port: int) | host: str, port: int               | -                  | Конструктор класса.   |0.0|
| start(self) -> None    | -                                            | -                  | Метод, который запускает сервер и ожидает запросы на обработку данных.     | infinity |
| socket_listen(self) -> Union[Exception, None] | -                     | Union[Exception, None] | Метод, который связывает сервер с IP-адресом и портом и начинает прослушивание входящих запросов.      |0.0|
| handle_client(self, client_socket, address) -> None | client_socket: socket, address: tuple | -    | Метод, который обрабатывает запросы на обработку данных от клиентов. |0.0|
